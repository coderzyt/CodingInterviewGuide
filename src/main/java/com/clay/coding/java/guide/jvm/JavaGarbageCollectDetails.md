...
Java的自动内存管理主要时针对对象内存的回收和对象内存的分配。同时，Java自动内存管理最核心的功能时**堆**内存当中对象的分配和回收。

从垃圾回收的角度来说，由于现在收集器都采用分代垃圾收集算法，所以Java堆被划分为了几个不同的区域，这样我们就可以根据各个区域的特点选择合适的垃圾收集算法。
...
![](http://javaguide.cn/assets/rf-hotspot-vm-gc.69291e6e.png)
...
这个方法实现简单，效率高，但是目前主流的虚拟机中并没有选择这个算法来管理内存，其主要原因时它很难解决对象之间相互循环引用的问题。
...
![可达性分析算法](http://javaguide.cn/assets/jvm-gc-roots.39234b20.png)
...
![](http://javaguide.cn/assets/%E6%A0%87%E8%AE%B0-%E6%B8%85%E9%99%A4%E7%AE%97%E6%B3%95.c1fbd0fe.jpeg)
...
![复制算法](http://javaguide.cn/assets/90984624.e8c186ae.png)
...
![标记-整理算法 ](http://javaguide.cn/assets/94057049.22c58294.png)
...
![ Serial 收集器 ](http://javaguide.cn/assets/46873026.3a9311ec.png)
...
![ParNew 收集器 ](http://javaguide.cn/assets/22018368.df835851.png)
...
![Parallel Scavenge 收集器 ](http://javaguide.cn/assets/22018368.df835851.png)
...
```java
-XX:G1ConcRefinementThreads=10 -XX:GCDrainStackTargetSize=64 -XX:InitialHeapSize=129829504 -XX:MaxHeapSize=2077272064 -XX:+PrintCommandLineFlags -XX:ReservedCodeCacheSize=251658240 -XX:+SegmentedCodeCache -XX:+UseCompressedClassPointers -XX:+UseCompressedOops -XX:+UseG1GC
...
```java
-XX:ConcGCThreads=3 -XX:G1ConcRefinementThreads=10 -XX:GCDrainStackTargetSize=64 -XX:InitialHeapSize=267690624 -XX:MarkStackSize=4194304 -XX:MaxHeapSize=4283049984 -XX:MinHeapSize=6815736 -XX:+PrintCommandLineFlags -XX:ReservedCodeCacheSize=251658240 -XX:+SegmentedCodeCache -XX:+UseCompressedClassPointers -XX:+UseCompressedOops -XX:+UseG1GC -XX:-UseLargePagesIndividualAllocation
...
![CMS 垃圾收集器 ](http://javaguide.cn/assets/CMS%E6%94%B6%E9%9B%86%E5%99%A8.8a4d0487.png)
